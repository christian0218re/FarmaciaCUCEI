...
def createSellWindow(userId):
    ...
    def agregar_producto(productolist):
        ...
        if producto and cliente_id_global is not None:
            ...
            importe = cantidad_a_agregar * precioProducto  
    ...
    def seleccionar_cliente(event):
        if listbox.curselection():  
            ...
            cliente_search_entry.delete(0, tk.END)  
            listbox.delete(0, tk.END)  
    ...
    tk.Label(productos_frame, text='Buscar Productos').grid(row=0, column=0, padx=5, pady=5, sticky='e')  # Alineación a la derecha
    ...
    def actualizar_stock():
        ...
        for item in tree.get_children():
            ...
            cantidad_comprada = int(item_values[3])  
    ...
    def ticket():
        ...
        cursor.execute("SELECT nombre FROM clientes WHERE clienteId = ?", (cliente_id_global,))
        nombreCliente = cursor.fetchone()[0]
        cursor.execute("SELECT puntos FROM clientes WHERE clienteId = ?", (cliente_id_global,))
        puntosCliente = cursor.fetchone()[0]
        cursor.execute("SELECT rfc FROM clientes WHERE clienteId = ?", (cliente_id_global,))
        puntosCliente = cursor.fetchone()[0]
        ...
        pdf.set_font("Arial", size=12)
        pdf.cell(200, 10, txt=f"Cliente ID: {cliente_id_global}  Nombre: {nombreCliente} Puntos: {puntosCliente}", ln=True)
        ...
        nuevos_puntos = int(subtotal // 100) 
